# Apps (né Apps v2) — RFC / PRD

> Git-backed filesystem, real shell, real builds, and open editing for Mako apps.
>
> NAMING (2026-08-30): after the v1 rip-out (§13.21) the "v2" qualifier was
> dropped everywhere — see §13.23. This document keeps its historical "v2"
> wording in dated sections; read "Apps v2" as today's "Apps".
...
| Viewer roles inside an app (DECIDED, built) | **Mongo decides who may open AND who is what; the repo decides what a role sees.** roles are a MEMBER attribute — job role + country, set by an admin on the Members page (`WorkspaceMember.jobRole`/`.country`), never in a repo; the repo only declares what a role may see, binding front matter `roles:` / `row_filter_<role>:` with `{{ viewer.<claim> }}` bound as parameters; enforced per viewer in `serveDeploymentFile` (filtered parquet streamed server-side via DuckDB, never the bucket redirect); the `pub.` grant carries the viewer; no viewer on a role-scoped app → 403. Repo config narrows, never grants. Detail: §27. | User (2026-09-02) |
...
## §13.23 The rename: v2 → the only apps system (2026-08-30)

With v1 gone from the write path, "v2" stopped qualifying anything, so the
qualifier was dropped across code, tools, routes, env, and data:

- Code: `api/src/apps-v2/` → `api/src/apps/`, `AppProjectV2`/`AppWorktreeV2`
  models → `AppProject`/`AppWorktree`, frontend `AppV2*`/`appsV2Store` →
  `App*`/`appsStore`, tab kinds `app-v2*` → `app*` (persisted tabs migrate on
  rehydrate), deep links `/a2/…` → `/a/…` (old links rewritten on load).
- Tools: `app2_*` → `app_*`; the freed v1 `app_*` names now belong to the
  git-backed toolset. The dead v1 `apps` system skill was deleted and the
  `apps-v2` skill renamed into its place; MCP initialize instructions teach
  only this loop now.
- Routes: `/api/workspaces/:id/apps-v2` → `…/apps`, `/api/apps-v2-{git,box,preview}`
  → `/api/apps-{git,box,preview}`. `/api/apps-v2-git` and `/api/apps-v2-box`
  stay mounted as aliases: live sandboxes have the old remote URL baked into
  their clones until recycled.
- Env: `APPS_V2_*` → `APPS_*`, with the old names still read as fallbacks
  (deployed environments and developer `.env` files were provisioned under
  them). Default disk roots moved `~/.mako/apps-v2/*` → `~/.mako/apps/*`,
  preferring the old directory when it already exists. The E2B template
  artifact keeps its published `mako-apps-v2` alias until rebuilt under
  `mako-apps` (deploy-app.yml pins it; the build script's default alias is
  already the new name).
- Data: migration `2026-08-30-120000_apps_v2_rename` renames collections
  `app_projects_v2`/`app_worktrees_v2` → `app_projects`/`app_worktrees` and
  workspace fields `settings.appsV2Enabled`/`appsV2Repo`/`appsV2CloudRepo` →
  `settings.appsEnabled`/`appsRepo`/`appsCloudRepo`. v1 data stays untouched
  per §13.21 (MakoApp docs, entity_versions, public-share serving).
...
> **Superseded in part, 2026-09-07 pm — roles are member attributes.** Théo,
> on the two repo-side designs below (member lists, then a roster `source`
> binding): *"Can't we simplify this? I want to edit the role in the mako
> UI. it's straightforward i should have sdr, bdr, csm, head of csm, team
> leader, developer, admin. and a country to select. no need to over
> engineer."* So: `WorkspaceMember.jobRole` + `.country` (and the same on
> invites, copied at acceptance), edited on the Members page by an admin,
> `PUT /workspaces/{id}/members/{userId}` taking `role` / `jobRole` /
> `country`. The published app gets `{email, role, country}` as claims from
> one Mongo read per document/data request; there is NO `viewers` block in
...
> and ignore `viewers`/`members`/`source`.
...
**Roles as data — `source` (2026-09-07).** The first cut put member emails
in `mako.json`, and the first real app refused to: "I don't want to
hardcode ANYTHING". The roster already exists in the warehouse (Close's
group memberships, synced daily), so the manifest may name a binding of the
app instead — `"source": "fr_viewers"` — whose rows are `email`, `role`
and any other column as a claim. Resolution order becomes static `members`
→ the viewer's source row → `default`. A promotion in the CRM changes the
view the next morning with no commit; a new rep's board exists the day
their CRM user lands in the group. Two things fail closed: a source row
naming a role `roles` never declared (the roster and the repo disagree),
and a source that was never materialized (a missing roster is not "no
roles"). The source is looked up per request the same way filtered rows
are — pulled from the store into an in-memory DuckDB — so it costs one
more small artifact read per document/data request; the same
`(artifact, policy)` cache follow-up covers it. Give the source binding
`-- roles: team_lead` so reps cannot read the roster itself.
