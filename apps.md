...
| GitHub App for workspace repo linking | **Mako** (`https://github.com/apps/mako`, private). Workspace content lives in the customer's connected repo. `Mako Transforms` / `Mako Transforms DEV` are retired; rotate `GITHUB_APP_*` secrets onto Mako and uninstall those two apps (org-owner follow-up). Mako Cloud Storage (`mako-cloud-storage`) remains the cloud-tier repo factory. | User (2026-09-02), issue #956 |
