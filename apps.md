...
| Viewer roles inside an app (DECIDED, built) | **Mongo decides who may open; the repo decides what an admitted viewer sees.** `mako.json` `viewers` (role names + optional default; membership from static `members` or, with nothing hardcoded, a `source` binding whose rows are email/role/claims), binding front matter `roles:` / `row_filter_<role>:` with `{{ viewer.<claim> }}` bound as parameters; enforced per viewer in `serveDeploymentFile` (filtered parquet streamed server-side via DuckDB, never the bucket redirect); the `pub.` grant carries the viewer; no viewer on a role-scoped app → 403. Repo config narrows, never grants. Detail: §27. | User (2026-09-02) |
