...
import { personalFolderRoutes } from "./personal-folders";
...
export function registerApiRoutes(app: OpenAPIHono<AuthEnv>): void {
  ...
  // Per-user grouping of an explorer's entities. Deliberately NOT part of the
  // apps surface: it stores only one person's view and can never change an
  // app's identity, sharing or deployment.
  app.route(
    "/api/workspaces/:workspaceId/personal-folders",
    personalFolderRoutes,
  );
