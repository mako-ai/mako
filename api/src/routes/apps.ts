/**
 * Apps routes — git-backed apps (see apps.md).
 *
...
export const appsRoutes = createRouter();
...
appsRoutes.use("*", unifiedAuthMiddleware);

appsRoutes.use("*", async (c: AuthenticatedContext, next) => {
...
async function loadProject(
  ...
  { project: IAppProject; userId?: string } | { errorResponse: Response }
> {
  const workspaceId = c.req.param("workspaceId");
...
// ---------------------------------------------------------------------------
// Status probe — Apps is always available (no feature flag). The frontend
...
appsRoutes.openapi(
  createRoute({
    ...
    path: "/status-probe",
    tags: ["Apps"],
    summary: "Apps availability for this workspace",
...
appsRoutes.openapi(
  createRoute({
    ...
    path: "/{id}",
    tags: ["Apps"],
    summary: "Delete an Apps project (repo included)",
    security: AUTH_SECURITY,
    request: { params: ProjectParam },
    responses: OPEN_RESPONSES,
  }),
      ...
      return c.json(
        {
          success: true as const,
          token: grant.token,
          url: `/api/apps-preview/${grant.token}/`,
...
appsRoutes.openapi(
  ...
  async c => {
    try {
      ...
      // just shows main ahead of what is deployed. (apps.md §13.3.1)
      try {
...
appsRoutes.openapi(
  createRoute({
    ...
    tags: ["Apps"],
    summary: "A stored app_browse screenshot",
    description:
