...
realtimeRoutes.openapi(
  createRoute({
    ...
    path: "/",
    tags: ["Realtime"],
    summary: "Subscribe to workspace realtime events (SSE)",
    description:
      "Server-Sent Events stream of workspace realtime events. Session auth only (EventSource cannot send API-key headers).",
    security: [{ cookieAuth: [] }],
    request: {
      params: z.object({
        workspaceId: z
      ...
      query: z.object({ clientId: z.string().optional() }),
    },
    responses: {
      ...OPEN_RESPONSES,
      200: {
