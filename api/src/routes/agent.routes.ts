 ...
 * every client-side tool round-trip — the assistant appears frozen for seconds
 * even though the tool itself resolved in milliseconds.
 *
...
agentRoutes.post("/chat", async (c: AuthenticatedContext) => {
  ...
  return result.toUIMessageStreamResponse({
    ...
    onFinish: ({ messages: allMessages }) => {
      // Run finalization in the background so it does not block the UI message
      // stream from closing. Blocking here would delay the client's tool-result
      // auto-resume by the full duration of cost computation + persistence,
      // making fast client-side tools feel frozen for seconds. See
