  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("process-event", async () => {
      try {
        ...
        const isEntityEnabled = hasEntityLayouts
          ? Boolean(entityLayout && entityLayout.enabled !== false)
          : true;

        // Skip disabled/not-selected entities before CDC enqueue/defer logic so
        // they never create CDC rows or destination tables.
        if (!isEntityEnabled) {
          await WebhookEvent.updateOne(
            { _id: webhookEvent._id },
            {
