/**
 * Apps v2 repository service — git substrate unit tests.
 *
 * Pure git-on-disk: no Mongo, no network. Run with
 * `pnpm --filter api run test:apps-v2`.
 */
...
beforeEach(async () => {
  tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "apps-v2-repo-test-"));
...
describe("initRepo + reads", () => {
  it("creates a bare repo seeded with the scaffold on main", async () => {
    const scaffold = createAppsV2Scaffold({ title: "Test App" });
