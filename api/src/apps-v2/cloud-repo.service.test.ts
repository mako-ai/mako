 ...
 *
 * Real git against file:// remotes (APPS_V2_GITHUB_REMOTE_BASE); Mongo and
...
vi.mock("../database/workspace-schema", () => ({
  Workspace: {
    findById: () => ({
      select: () => ({
        lean: async () => ({ appsV2CloudRepo: state.pointer }),
      }),
    }),
...
beforeAll(async () => {
  ...
  process.env.APPS_V2_GITHUB_REMOTE_BASE = `file://${remotesRoot}`;
});
...
afterEach(() => {
  delete process.env.APPS_V2_CONNECTED_REPO_PUSH;
});
...
describe("resolveMirrorTarget", () => {
  ...
  it("treats the binding as inert metadata where the tier is gated off", async () => {
    delete process.env.APPS_V2_CONNECTED_REPO_PUSH; // prefix "dev" → gated
    state.binding = { owner: "acme", repo: "site" };
    state.pointer = { owner: "mako-ai-cloud", repo: "dev-x" };
    const target = await resolveMirrorTarget(workspaceId);
