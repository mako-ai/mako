/**
 * Apps v2 durable mirrors — where a workspace repo's history is replicated.
 *
 ...
 * explicit APPS_V2_CONNECTED_REPO_PUSH=allow opt-in. Previews and dev run on
...
function remoteBase(): string {
  return process.env.APPS_V2_GITHUB_REMOTE_BASE || "https://github.com";
}
...
export function connectedTierEnabled(): boolean {
  return (
    getMakoCloudRepoPrefix() === "ws" ||
    process.env.APPS_V2_CONNECTED_REPO_PUSH === "allow"
  );
}
...
async function cloneMirrorInto(
  ...
  try {
    ...
    logger.info("Apps v2 local repo restored from its mirror", {
      workspaceId,
      repo: label,
    });
  } finally {
...
export function queueMirrorPush(workspaceId: string): void {
  ...
  void schedulePush(workspaceId).catch(error => {
    logger.warn("Apps v2 mirror push failed", {
      workspaceId,
...
export async function fetchFromCloud(
  ...
  if (fastForwards) {
    ...
    logger.warn(
      "Apps v2 connected-repo fetch skipped: local branch has diverged from the remote",
...
export async function adoptConnectedRepo(
  ...
  if (!(await repoExists(repoDir))) {
    ...
    if (pointer && isMakoCloudConfigured()) {
      await cloneMirrorInto(
        ...
        logger.warn("Apps v2 legacy mirror restore failed during adoption", {
          workspaceId,
          error: error instanceof Error ? error.message : String(error),
        });
      });
  ...
  if (!remoteEmpty) {
    ...
    logger.info("Apps v2 workspace imported its connected repo", {
      workspaceId,
      repo: label,
    });
    return "imported";
  ...
  if (hasHistory) {
    ...
    logger.info("Apps v2 workspace history seeded into its connected repo", {
      workspaceId,
      repo: label,
    });
    return "seeded";
