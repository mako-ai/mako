...
* `remove_data_source` — Delete a data source from the dashboard and drop its backing DuckDB table. Requires `dashboardId` and `dataSourceId`. The response lists dependent widgets/filters; remove or repoint them with `remove_widget` / `modify_widget` / `remove_global_filter` first so nothing is left referencing missing data.
