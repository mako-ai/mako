---
name: dashboards
description: Creating, modifying, or debugging Mako dashboards — data sources, widgets (charts/KPIs/tables), cross-filtering, edit lifecycle, and the dashboard tool list. Load when the user works on dashboards or widgets.
entities:
  - dashboard
  - dashboards
  - widget
...
When working with dashboards, you help users create interactive data dashboards
from their saved queries (consoles).

## Core capabilities

You can create, modify, and manage dashboards using structured tool calls.
Dashboards consist of:
- **Data sources** — dashboard-local query definitions materialized into an
  in-browser DuckDB instance
- **Widgets** — charts (Vega-Lite), KPI cards, and data tables that query the
  local DuckDB data
- **Cross-filtering** — clicking a bar or slice in one chart filters all other
  charts automatically
- **Global filters** — dashboard-level date range pickers, dropdowns, and
  search fields

## Multi-dashboard support

Multiple dashboards can be open simultaneously, each with its own isolated
DuckDB instance. **You MUST pass `dashboardId` to every tool that operates on a
dashboard.** There is no implicit "current dashboard" — always be explicit.
...
Before making any changes to a dashboard, you MUST call `enter_edit_mode` with
the target `dashboardId`. This acquires the edit lock and puts the dashboard
into edit mode.
...
For detail beyond the core workflow, read the matching reference file with
`read_skill_resource`:
- `references/duckdb-sql.md` — DuckDB SQL reference (timestamp handling, functions, casting). Read this before writing any `localSql` or data source query.
- `references/widgets-and-charts.md` — Chart guidelines, layout/sizing rules, and worked widget examples (area/bar/line/donut/KPI specs).
- `references/cross-filtering.md` — Cross-filter rules, debugging order, temporal cross-filter handling, source query edit safety, and query failure triage. Read this before debugging any cross-filter or interaction bug.
