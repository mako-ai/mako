...
const CLOSE_ENTITY_FIELD_HINTS: Record<string, string[]> = {
  leads: [
    "id",
    "display_name",
    "status_id",
...
const CLOSE_LAYOUT_DATE_CREATED = {
  partitionField: "date_created",
  partitionGranularity: "day" as const,
  clusterFields: [] as string[],
};

const CLOSE_LAYOUT_SYNCED_AT = {
  partitionField: "_syncedAt",
  partitionGranularity: "day" as const,
  clusterFields: [] as string[],
};

function getCloseLayoutSuggestion(entityName: string) {
  const syncedAtEntities = new Set([
    "custom_fields",
    "custom_activity_types",
    "custom_object_types",
    "lead_statuses",
  ...
  return syncedAtEntities.has(entityName)
    ? CLOSE_LAYOUT_SYNCED_AT
    : CLOSE_LAYOUT_DATE_CREATED;
}
...
export class CloseConnector extends BaseConnector {
  ...
  getEntityMetadata(): EntityMetadata[] {
    const withDefaults = (
      entityName: string,
      base?: Partial<EntityMetadata>,
    ) => ({
      name: entityName,
      label: base?.label,
      description: base?.description,
      layoutSuggestion: getCloseLayoutSuggestion(entityName),
      fieldHints: CLOSE_ENTITY_FIELD_HINTS[entityName],
    });

    return [
      withDefaults("leads", { label: "Leads" }),
      withDefaults("opportunities", { label: "Opportunities" }),
      {
        name: "activities",
        label: "Activities",
        description: "All activity types from Close.com",
        layoutSuggestion: CLOSE_LAYOUT_DATE_CREATED,
        subEntities: CLOSE_ACTIVITY_TYPES.map(type => ({
          name: type.name,
          label: type.label,
          description: type.description,
          layoutSuggestion: getCloseLayoutSuggestion(`activities:${type.name}`),
          fieldHints: CLOSE_ENTITY_FIELD_HINTS[`activities:${type.name}`],
        })),
      },
      withDefaults("contacts", { label: "Contacts" }),
      withDefaults("users", { label: "Users" }),
      withDefaults("custom_fields", { label: "Custom Fields" }),
      withDefaults("custom_activity_types", { label: "Custom Activity Types" }),
      withDefaults("custom_object_types", { label: "Custom Object Types" }),
      withDefaults("custom_objects", { label: "Custom Objects" }),
      withDefaults("lead_statuses", { label: "Lead Statuses" }),
      withDefaults("opportunity_statuses", { label: "Opportunity Statuses" }),
    ];
  }
