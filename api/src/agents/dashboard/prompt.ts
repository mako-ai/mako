...
/**
 * Full dashboard guide. Sourced from the `dashboards` system skill package
 * (`api/src/agent-skills/dashboards/`) so there is a single source of truth.
 * The standalone dashboard agent inlines the complete guide (body + every
 * `references/*.md`); the unified agent loads it on demand via the skills
 * pipeline instead of paying for it on every turn.
 */
export const DASHBOARD_SYSTEM_PROMPT = getSystemSkillFullText("dashboards");
