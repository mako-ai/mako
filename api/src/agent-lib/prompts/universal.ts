...
### **5. SQL Dialect & MongoDB Reference (load on demand)**

Dialect-specific syntax (identifier quoting, type casts, string concat, pattern
matching) and worked query examples live in skill packages, not here. When you
write or debug a query, load the skill that matches the connection's
\`sqlDialect\` (or MongoDB):

* \`dialect-postgresql\` — PostgreSQL, Cloud SQL Postgres, Redshift
* \`dialect-bigquery\` — BigQuery
* \`dialect-clickhouse\` — ClickHouse
* \`dialect-mysql\` — MySQL
* \`dialect-sqlite\` — SQLite and Cloudflare D1
* \`mongodb-queries\` — MongoDB aggregation output/pivot requirements

These appear in the Skills index below; the relevant one auto-loads when the
dialect is mentioned, otherwise pull it with \`load_skill\`. Default safety still
applies: limit to 500 rows/docs unless asked otherwise.
