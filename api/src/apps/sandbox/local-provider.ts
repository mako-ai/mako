...
import { appsSessionsRoot } from "../config";
...
const logger = loggers.api("apps-local-sandbox");
 ...
 */
const SANDBOX_HOME = path.join(os.tmpdir(), "mako-apps-cache", "home");
...
function assertNotProduction(): void {
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "The local Apps sandbox runs tenant commands in the API process and " +
        "must never be used in production. Unset APPS_SANDBOX_PROVIDER or " +
        "set it to 'e2b'.",
    );
  }
}
...
export const localSandboxProvider: SandboxProvider = {
  ...
  async openTerminal(): Promise<SandboxTerminal> {
    ...
    throw new Error(
      "The local sandbox has no interactive terminal — use the E2B provider " +
        "(APPS_SANDBOX_PROVIDER=e2b) to open one.",
    );
  },
  ...
  async destroySession(sessionKey) {
    ...
    logger.info("Local Apps sandbox destroyed", { sessionKey });
  },
};
