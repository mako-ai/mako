 ...
 * `bindings/<name>.sql` files with front matter, plus an AppProject row for
...
const logger = loggers.api("apps-migrate-v1");
...
export async function clearWorkspaceV2Apps(workspaceId: string): Promise<{
  ...
  const projects = await AppProject.find({ workspaceId: ws });
 ...
 * the AppProject row under the DERIVED id (same id every folder-only code
...
export async function backfillV1AppRow(
  ...
  const existing = await AppProject.findOne({ _id: id });
  if (!existing) {
    try {
...
export async function migrateWorkspaceV1Apps(input: {
  ...
  if (input.execute && input.reset && !input.appId) {
    ...
    logger.info("Apps migration reset: cleared existing apps", {
