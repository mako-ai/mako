...
const logger = loggers.api("apps-box-state");
...
function redisStore(url: string): StateStore {
  ...
  redis.on("error", error =>
    logger.warn("Apps box-state redis error", {
...
function keyFor(sessionKey: string): string {
  return `apps:box-state:${sessionKey}`;
}
...
export async function getBoxState(
  ...
  try {
    ...
    logger.warn("Apps box-state read failed", {
      sessionKey,
      error: error instanceof Error ? error.message : String(error),
    });
    return null;
...
export async function patchBoxState(input: {
  ...
  if (patch.devServer && prev?.devServers == null) {
    try {
      ...
      logger.warn("Apps box-state could not seed dev servers", {
        sessionKey,
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  if (next.devServers) {
    ...
    for (const server of next.devServers) {
      ...
      try {
        ...
        logger.warn("Apps box-state could not resolve a dev-server url", {
          sessionKey,
          slug: server.slug,
          error: error instanceof Error ? error.message : String(error),
        });
  ...
  try {
    ...
    logger.warn("Apps box-state write failed", {
      sessionKey,
      error: error instanceof Error ? error.message : String(error),
    });
  }

  logger.debug("Apps box-state patched", {
    sessionKey,
    source,
    branch: next.branch,
