/**
 * Interactive terminal for Apps, over a WebSocket.
 *
...
const logger = loggers.api("apps-terminal");

const PATH_RE = /^\/api\/workspaces\/([^/]+)\/apps\/([^/]+)\/terminal(\?.*)?$/;
...
async function authorize(
  ...
  workspaceId: string,
  appRef: string,
): Promise<{ project: IAppProject; userId: string } | null> {
  ...
  return { project: project as IAppProject, userId: user.id };
}

export function attachAppsTerminalWs(serverType: ServerType): void {
  ...
  logger.info("Apps terminal WebSocket attached");
}
...
function reattach(session: LiveTerminal, ws: WebSocket): void {
  ...
  logger.info("Apps terminal ring replayed", { bytes: replayed });
}
...
async function startSession(
      ...
      try {
        ...
        const flush = () => {
          ...
          if (created.sockets.size === 0) {
            logger.info("Apps terminal output with no sockets", {
              termId,
              bytes: chunk.length,
            });
          }
        ...
        session = created;
        logger.info("Apps terminal started", {
          termId,
  ...
  const detach = () => {
    ...
    current.reaper = setTimeout(() => {
      ...
      logger.info("Apps terminal reaped after no clients", { key });
    }, ORPHAN_GRACE_MS);
  };
  ws.on("close", detach);
  ws.on("error", detach);
