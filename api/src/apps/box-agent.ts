...
const logger = loggers.api("apps-box-agent");
...
async function tick() {
  ...
  // Idle reaper (apps.md §13.9): the box is the authority for liveness and
  ...
  warnedEnv = false;
  try {
    const res = await fetch(e.MAKO_API + "/api/apps-box/" + e.MAKO_WS + "/events", {
      method: "POST",
...
export async function installBoxAgent(ctx: SandboxExecContext): Promise<void> {
  ...
  if (installed.stdout.includes("env-missing")) {
    ...
    await configureBoxRemote({
      ...
      logger.warn("Apps box env could not be restored", {
...
export async function ensureBoxAgent(
  ...
  try {
    ...
    logger.warn("Apps box agent could not be installed", {
      sessionKey: ctx.sessionKey,
      error: error instanceof Error ? error.message : String(error),
    });
  }
