...
describe("v1 → v2 migration", () => {
  it("moves files, converts bindings, carries access — and says what it could not do", async () => {
    ...
    const project = await AppProject.findById(result.projectId);
  ...
  it("replays every saved version as a git commit — author, date, message", async () => {
    ...
    const rows = log.map(l => {
      ...
      // git's strict-ISO (%aI/%cI) prints UTC as "Z" or "+00:00" depending
      // on the git version; normalize so the assertions hold on both.
      return { hash, rest: rest.join("|").replace(/\+00:00/g, "Z") };
    });
  ...
  it("is idempotent by overwrite: a second run replaces in place, no duplicates", async () => {
    ...
    const { MakoApp, AppProject } = await import(
    ...
    const beforeCount = await AppProject.countDocuments({
      workspaceId: new Types.ObjectId(WS),
    });
    ...
    expect(
      await AppProject.countDocuments({
        workspaceId: new Types.ObjectId(WS),
      }),
    ).toBe(beforeCount);
