/**
 * The dbt mark, exactly as the dbt Power User VS Code extension ships it for
 * its activity bar (media/images/dbt_icon.svg) — VS Code renders that SVG as
 * a monochrome mask, which is the "outlined" look in its sidebar. Same idea
 * here: one path, filled with currentColor, so it takes the rail's color the
 * way every lucide icon does.
 *
 * A forwardRef with lucide's prop type, because everything downstream (the
 * icon registry, the command palette) types icons as LucideIcon and this has
 * to be substitutable for one.
 *
 * OUTLINED, not filled: the mark's contour is stroked instead of painted,
 * which is the look the extension's sidebar actually has — and the look
 * every lucide neighbour in the rail has. `non-scaling-stroke` makes the
 * incoming strokeWidth mean screen pixels despite the 90×81 viewBox, so the
 * line weight matches the icons beside it exactly.
 */
import { forwardRef } from "react";
import type { LucideIcon, LucideProps } from "lucide-react";

const DbtIcon = forwardRef<SVGSVGElement, LucideProps>(function DbtIcon(
