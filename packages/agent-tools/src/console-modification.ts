/**
 * Console modification engine — single source of truth for how console
 * content modifications (replace / append / insert / patch) are applied and
 * how their compact diff previews are built.
 *
 * Shared between:
 *   - the browser (Monaco-side application + tool cards), via the
 *     `app/src/utils/consoleModification.ts` re-export shim, and
 *   - the API (server-side modify_console execution against the
 *     authoritative SavedConsole draft).
 *
 * Pure string-in/string-out: no DOM, no Monaco, no Mongo.
 */
